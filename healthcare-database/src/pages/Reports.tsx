              ...
              <tbody className="divide-y divide-sage-200">
                {teamPulses.map((team, idx) => (
                  <tr key={idx} className="hover:bg-sage-50 transition-colors">
                    ...
                    {metrics.map((metric) => {
                      const val = team[metric.key as keyof typeof team] as number | null | undefined
                      return (
                        <td key={metric.key} className="px-4 py-4 text-center">
                          <span className={`inline-block px-3 py-1 rounded font-semibold text-sm ${getScoreColor(val ?? 0)}`}>
                            {fmt(val)}
                          </span>
                        </td>
                      )
